...
(function() {
    ...
    Window_Options.prototype.addCustomOptions = function() {
        iterate(this._customParams, function(key, item) {
            ...
            if (item.name == 'Ｈ時のテキストスキップ' && !$gameSwitches.value(73)) {
               if (!ConfigManager.hiddenInfo[key]) this.addCommand('Skip text at H', key);
            } else if (item.name == 'エッチシーン自体をスキップ' && !$gameSwitches.value(73)) {
               if (!ConfigManager.hiddenInfo[key]) this.addCommand('Skip H scene', key);
            } else if (item.name == '戦闘アクションのスキップ' && !$gameSwitches.value(73)) {
               if (!ConfigManager.hiddenInfo[key]) this.addCommand('Skip combat action', key);
            } else {
               if (!ConfigManager.hiddenInfo[key]) this.addCommand(item.name, key);
             }
